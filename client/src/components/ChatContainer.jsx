...
const ChatContainer = () => {
  ...
  return selectedUser? (
    ...
    <img src={assets.help_icon} alt='' className="max-md:hidden max-w-5"/>
      ...
      {/*------- chat area------*/}
      <div className='flex-1 flex flex-col overflow-y-scroll p-3 pb-24'>
        {messages.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId!==authUser._id && 'flex-row-reverse'}`}>
            ...
            <div className='text-center text-xs'>
              <img 
              src={msg.senderId===authUser._id? authUser?.profilePic|| assets.avatar_icon: selectedUser?.profilePic ||assets.avatar_icon} alt='' className='w-7 rounded-full'/>
